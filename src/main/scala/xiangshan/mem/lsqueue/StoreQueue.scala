...
class StoreQueue(implicit p: Parameters) extends XSModule
  ...
  for (i <- 0 until StoreQueueSize) {
    needCancel(i) := (uop(i).robIdx.needFlush(io.brqRedirect) && !vecExceptionFlag.valid || isAfter(uop(i).robIdx, io.brqRedirect.bits.robIdx) && io.brqRedirect.valid && vecExceptionFlag.valid) && allocated(i) && !committed(i)
    when (needCancel(i)) {
      allocated(i) := false.B
      completed(i) := false.B
    }
